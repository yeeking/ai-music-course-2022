...
def synth_samples(targets, predictions, vocoder, model_config, preprocess_config, path='./', filename='sing'):
    ...
    for wav, basename in zip(wav_predictions, basenames):
        ...
        wavfile.write(wav_filename, sampling_rate, wav)
    
    return wav_filename, len(wav_predictions[0]) / float(sampling_rate)
