...
class VarianceAdaptor(nn.Module):
    ...
    def get_pitch_embedding(self, x, target, mask, control):
        ...
        print ("modules.py get_pitch_embedding: pitch target is ", target )
    ...
    def forward(
        ...
        print("modules.py::forward x shape ", x.shape)
        # now convert the pitch target tensor to account for x.shape[1] which is the
        # number of phonems. How does 40 relate to the eventual number? 
        # not clear yet
        ...
        if self.pitch_feature_level == "frame_level":
            ...
            if len(x) != len(pitch_embedding):
                print("modules.py: foward pitch embedding length is wrong, stretch it")
                print("modules.py: forward. x shape ", x.shape)
                print("modules.py: forward. pitch embedding shape ", pitch_embedding.shape)

                want_rows = x.shape[1]
                got_rows = pitch_embedding.shape[0]
                reps = int(want_rows / got_rows)
                
                print("modules.py want ", want_rows, "got ", got_rows, " reps ", reps)
                pitch_embedding = torch.repeat_interleave(pitch_embedding, reps, dim=0)
                if pitch_embedding.shape[0] < want_rows:
                    a = np.resize(pitch_embedding.cpu(), [want_rows, pitch_embedding.shape[1]])
                    pitch_embedding = torch.tensor(a, device=device)

                # need to map pttch embedding to the length of x
                print("modules.py: forward. x shape ", x.shape)
                print("modules.py: forward. pitch embedding shape ", pitch_embedding.shape)
                # print("X: ", x)
                # print("embedding:", pitch_embedding)
